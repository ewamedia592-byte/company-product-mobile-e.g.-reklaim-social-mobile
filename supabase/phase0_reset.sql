...
as $$
begin
    insert into public.chat_thread_participants (thread_id, user_id, display_name, avatar_url, role, invited_by)
    values (
        p_thread_id,
        p_user_id,
    ...
    on conflict (thread_id, user_id) do update
        set display_name = excluded.display_name,
            ...
            role = coalesce(excluded.role, chat_thread_participants.role),
            invited_by = coalesce(excluded.invited_by, chat_thread_participants.invited_by);
end;
$$;
...
begin
    ...
    -- Ensure the sender belongs to the thread
    if not exists (
        select 1
          from public.chat_thread_participants tp
         where tp.thread_id = p_thread_id
...
begin
    ...
    perform public.chat_upsert_participant(invite_rec.thread_id, invite_rec.guest_id, 'guest', invite_rec.requested_by);
